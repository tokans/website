...
for (const d of dirs) {
  ...
  let tagline = null;
  const pkgPath = join(workspaceRoot, d.name, "package.json");
  ...
  await sql`
    INSERT INTO apps (slug, name, tagline, uses_sharedcorelib, support_status, listed)
    VALUES (${slug}, ${d.name}, ${tagline}, TRUE, 'listed', TRUE)
